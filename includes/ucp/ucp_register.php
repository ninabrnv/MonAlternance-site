...
class ucp_register
{
	...
	function main($id, $mode)
	{
		...
		$config['enable_confirm'] = false;	// désactivation catchapa
		...
		$coppa			= false; //$request->is_set('coppa') ? (int) $request->variable('coppa', false) : false;
		$agreed			= true; //$request->variable('agreed', false);
		...
		*/
		...
		$data = array(
			'username'			=> strtolower($request->variable('email', '')), // $request->variable('username', '', true),
		...
		// Récupération des informations de profil "alt"
		$dataAlt = array(
			'type_compte'	=> strtolower($request->variable('type_compte', '', true)),
			'prenom'	=> $request->variable('prenom', '', true),
			'nom'	=> strtoupper($request->variable('nom', '', true)),
			'ville'	=> strtoupper($request->variable('ville', '', true)),
			'ville_proche'	=> strtoupper($request->variable('ville_proche', '', true)),
			'region'	=> strtoupper($request->variable('region', '', true)),
			'date_naissance'	=> $request->variable('date_naissance', '', true),
			'domaine_principal'	=> $request->variable('domaine_principal', '', true),
			'domaine_artisanat'	=> $request->variable('domaine_artisanat', '', true),
			'niveau_etude'	=> $request->variable('niveau_etude', '', true),
			'derniere_ecole'	=> $request->variable('derniere_ecole', '', true),
			'tel_mobile'	=> $request->variable('tel_mobile', '', true),
			'societe_ident_prenom'	=> $request->variable('societe_ident_prenom', '', true),
			'societe_ident_nom'	=> strtoupper($request->variable('societe_ident_nom', '', true)),
			'societe_nom'	=> strtoupper($request->variable('societe_nom', '', true)),
			'societe_region'	=> strtoupper($request->variable('societe_region', '', true)),
			'societe_adresse'	=> $request->variable('societe_adresse', '', true),
			'societe_adresse_suite'	=> $request->variable('societe_adresse_suite', '', true),
			'societe_adresse_ville'	=> strtoupper($request->variable('societe_adresse_ville', '', true)),
			'societe_adresse_cp'	=> $request->variable('societe_adresse_cp', '', true),
			'societe_adresse_pays'	=> strtoupper($request->variable('societe_adresse_pays', '', true)),
			'societe_tel'	=> $request->variable('societe_tel', '', true),
			'societe_siret'	=> $request->variable('societe_siret', '', true),
			'societe_num_tva_intra'	=> $request->variable('societe_num_tva_intra', '', true),
			'societe_num_exo_tva'	=> $request->variable('societe_num_exo_tva', '', true),
			'societe_secteur_activite'	=> $request->variable('societe_secteur_activite', '', true),
			'societe_effectif'	=> $request->variable('societe_effectif', '', true)
		);
			...
			while ($row = $db->sql_fetchrow($result)) {
				...
				$template->assign_block_vars('villesDisponibles', array(
					...
					'VILLE_LABEL' => $row['ville_nom_reel'] . ' ('. substr($row['ville_code_postal'],0,5) .')',
		...
		*/
			...
			*/
			...
			if (!sizeof($error))
			{
				...
				*/
				...
				////////////////////////////
				// Alt : création du profil
				////////////////////////////
				$dataAlt['user_id'] = $user_id;
				$dataAlt['email'] = $data['email'];
			

				$alt_profil_id = alt_profil_add($dataAlt);
				
				if ($alt_profil_id === false)
				{
					trigger_error('Erreur de création de profil ALT', E_USER_ERROR);
				}
				...
				$message = $message . '<br /><br />' . sprintf("%sPage de connexion%s", '<a href="' . append_sid("{$phpbb_root_path}ucp.$phpEx?mode=login") . '">', '</a>');
		...
		$template->assign_vars(array(
			...
			'display_candidat' => (strtolower($request->variable('type_compte', '', true)) === 'candidat') ? '' : 'none',
			'display_societe' => (strtolower($request->variable('type_compte', '', true)) === 'entreprise') ? '' : 'none',
			'display_submit_form' => (strtolower($request->variable('type_compte', '', true)) !== '') ? '' : 'none',

			'type_compte_candidat_checked'	=> (strtolower($request->variable('type_compte', '', true)) === 'candidat') ? 'checked':'',
			'type_compte_societe_checked'	=> (strtolower($request->variable('type_compte', '', true)) === 'entreprise') ? 'checked':'',
			'prenom'	=> $request->variable('prenom', '', true),
			'nom'	=> strtoupper($request->variable('nom', '', true)),
			'ville'	=> strtoupper($request->variable('ville', '', true)),
			'ville_proche'	=> strtoupper($request->variable('ville_proche', '', true)),
